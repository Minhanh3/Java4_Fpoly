...
public class SachService {
    ...
    // co ngân
    public int searchTetg(String tenTacGia) {
        int dem = 0;
        for (Sach sach : sachList) {
            String ten = sach.getTenTacgia();
            if (ten.equalsIgnoreCase(tenTacGia)) {
                dem++;
            }
        }
        if (dem == 0) {
            throw new ArrayIndexOutOfBoundsException("khong co trong danh sach");
        } else {
            return dem;
        }
    }
